...
function LimitedTimeMonthGiftConfig:init()
  local config = Lib.read_csv_file(Root.Instance():getGamePath() .. "config/commercialize/limited_time_month_gift.csv", 3)
  config = config or Lib.read_csv_file(Root.Instance():getGamePath() .. "config/limited_time_month_gift.csv", 3) or {}
...
function LimitedTimeMonthGiftConfig:getCfgById(id)
  if not settings[id] then
    Lib.logError("can not find cfgLimitedTimeMonthGiftConfig, id:", id)
    return
  end
  return settings[id]
end

function LimitedTimeMonthGiftConfig:getAllCfgs()
  return Lib.copy(settings)
end

function LimitedTimeMonthGiftConfig:getCfgByActivityId(activityId)
  local result = {}
...
function LimitedTimeMonthGiftConfig:getCfgByRoundActivityId(activityId, roundId)
...
function LimitedTimeMonthGiftConfig:getCfgByActivityIdTotalRound(activityId)
  if not roundCfg[activityId] then
    return 1
  end
  local totalNum = 0
...
LimitedTimeMonthGiftConfig:init()
return LimitedTimeMonthGiftConfig
