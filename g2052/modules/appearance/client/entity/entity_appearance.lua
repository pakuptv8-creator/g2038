...
function Entity:refreshBoundBoxWithShapeScale()
  local DramaClientHelper = T(Lib, "DramaClientHelper")
  if DramaClientHelper:checkInTemplateMod(Define.DramaTemplateKey.Giant or 0) then
    self:updateBoundingVolume(self:getGiantShape())
  else
    self:updateBoundingVolume(self:getShapeScale())
  end
end

function Entity:parseNewSkinDataClient(skinDataChange)
...
local oldApplySkin = EntityClient.applySkin

function EntityClient:applySkin(skin, opOrder)
  if opOrder then
    if self.applySkinOpOrder and opOrder < self.applySkinOpOrder then
      return
    end
    self.applySkinOpOrder = opOrder
  end
  oldApplySkin(self, skin)
end
